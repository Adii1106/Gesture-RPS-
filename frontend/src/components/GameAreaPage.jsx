  ...
  useEffect(() => {
    ...
    const runDetection = async () => {
      ...
      if (hands.length > 0) {
        ...
        // =====================
        // HAND MARKER ALIGNMENT SECTION (adjust offsets here)
        // =====================
        ...
        // You can adjust these values to fine-tune marker alignment
        const offsetX = 120; // Move right (positive), left (negative)
        const offsetY = 100; // Move down (positive), up (negative)
        console.log('Offsets:', offsetX, offsetY, 'Scale:', scaleX, scaleY); // DEBUG: Check if this runs
        ctx.fillStyle = '#00FF00';
        keypoints.forEach(pt => {
          ctx.beginPath();
          ctx.arc(pt.x * scaleX + offsetX, pt.y * scaleY + offsetY, 4, 0, 2 * Math.PI);
          ctx.fill();
        });
        // =====================
        // END HAND MARKER ALIGNMENT SECTION
        // =====================
        ctx.strokeStyle = '#FF0000';
        ctx.lineWidth = 2;
        // Scale bounding box as well, with offsets
        const xs = keypoints.map(pt => pt.x * scaleX + offsetX);
        const ys = keypoints.map(pt => pt.y * scaleY + offsetY);
        let xMin = Math.max(0, Math.floor(Math.min(...xs) - PADDING));
        let xMax = Math.min(canvasWidth, Math.floor(Math.max(...xs) + PADDING));
        let yMin = Math.max(0, Math.floor(Math.min(...ys) - PADDING));
        let yMax = Math.min(canvasHeight, Math.floor(Math.max(...ys) + PADDING));
        let width = xMax - xMin;
