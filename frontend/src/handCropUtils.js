 ...
 * @param {boolean} generatePreview - When true, also generate a 300x300 preview data URL (default: false).
 * @returns {Promise<{tensor: tf.Tensor4D, previewDataUrl?: string}>} - The cropped, resized tensor and an optional preview image data URL.
 */
export async function cropHandFromVideo({ keypoints, canvas, video, padding = 30, targetSize = 224, generatePreview = false }) {
  ...
  tf.dispose([imgTensor, croppedImg]);
